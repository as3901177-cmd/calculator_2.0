...
def analyze_hanging_lines(
    ...
    for chain in piercing_details['chains']:
        if chain['type'] in ('open', 'isolated'):
            ...
            # Проверяем возможность замыкания: расстояние между концами первого и последнего объекта цепи
...
def _get_chain_endpoints(chain_objs: List[DXFObject]) -> Optional[Tuple[Tuple[float,float], Tuple[float,float]]]:
    ...
    Для этого используется порядок, определённый в piercing_counter (объекты в цепи уже упорядочены?).
    В `count_piercings_advanced` порядок не гарантирован, поэтому нужно построить граф и найти крайние точки.
    """
    # Простейший способ: взять все конечные точки объектов, найти две, у которых нет пары (или одна если цепь из одного)
    ...
    if not all_points:
        return None
    # Строим частотный словарь точек
    point_count = {}
    ...
    # Ищем точки, которые встретились нечётное количество раз
    odd_points = [pt for pt, cnt in point_count.items() if cnt % 2 != 0]
    if len(odd_points) == 2:
        return (odd_points[0], odd_points[1])
    # Если нечётных нет (замкнутая), возвращаем None
    return None
...
def generate_quality_report(
    ...
    # Замкнутые контуры
    closed_quality = check_closed_contours_quality(chain_polygons, fixed_polygons, external_id, internal_ids)
    report['closed_contours'] = closed_quality
    report['summary']['total_closed'] = len(closed_quality)
    report['summary']['valid_closed'] = sum(1 for v in closed_quality.values() if v['is_valid'] and not v['issues'])
    ...
    # Объекты без цепи (chain_id == -1)
    unassigned = [obj for obj in objects_data if obj.chain_id == -1]
    report['unassigned_objects'] = [{'num': obj.num, 'type': obj.entity_type, 'length': obj.length} for obj in unassigned]
