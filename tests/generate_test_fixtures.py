...
class TestFixturesGenerator:
    ...
    def create_slot(self):
        ...
        # Вершины (bulge будет установлен потом)
        vertices = [
            (r, 0),                     # 0: начало нижней дуги
            (w - r, 0),                 # 1: начало верхней дуги (после нижней прямой)
            (w - r, h),                 # 2: конец верхней дуги → начало верхней прямой
            (r, h)                      # 3: конец верхней прямой → начало нижней дуги
        ]

        lwp = msp.add_lwpolyline(vertices, close=True)

        # Явно задаём bulge для дуговых сегментов через доступ к вершинам
        # Индекс bulge соответствует начальной вершине сегмента
        lwp[1].bulge = 1   # сегмент 1→2 : верхняя полуокружность (выпуклая вверх)
        lwp[3].bulge = 1   # сегмент 3→0 : нижняя полуокружность (выпуклая вниз, замыкание)
